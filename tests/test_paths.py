...
旧代码把同一个 trial 的位置拆在两处（数据路径挂 Data、产物路径挂
...
def test_dataset_paths_shared_by_projection_key(exp):
    ...
    m = Data(type="dataset", source="parse/dataset/dataset_D.jsonl",
...
def test_dataset_projection_key_depends_on_eval_and_parse(exp):
    ...
    p0 = TrialPaths.for_trial(exp, 0, "t", Data(**base))
    assert TrialPaths.for_trial(exp, 0, "t", Data(**other_eval)).train_data != p0.train_data
    assert TrialPaths.for_trial(exp, 0, "t", Data(**other_fmt)).train_data != p0.train_data


def test_expr_path_encodes_all_knobs(exp):
    m = Data(type="expr", ops="+-", repeat=2, start=0, end=9,
                 input_format="infix", parse="post", eval="none", split=0.2)
...
def test_expr_ops_tag_avoids_filesystem_unsafe_chars(exp):
    m = Data(type="expr", ops="&|*/", repeat=2, start=0, end=9, parse="post")
...
def test_split_non_positive_means_same_file(exp):
    m = Data(type="expr", ops="+-", repeat=2, start=0, end=9, split=0.0)
    p = TrialPaths.for_trial(exp, 0, "t", m)
...
def test_artifact_names_are_id_based(exp):
    ...
    m = Data(type="expr", ops="+-", repeat=2, start=0, end=9, split=0.2)
...
def test_paths_live_under_experiment_dir(exp):
    m = Data(type="bead", start=0, end=4)
