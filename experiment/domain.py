"""实验域模型 —— Experiment 聚合根及其组成部分（Data / Method / Trial / TrialPaths）。

2026-09-18 拆分：
  - 模型架构配置（BrainConfig / PosEmbConfig / ModelConfig）→ `model/domain.py`
...
class TrialPaths:
    ...
    杜绝散落各处的字符串拼接（旧代码里 data 路径挂在 Data、
    ...
    def for_trial(cls, exp: "Experiment", trial_id: int, trial_name: str,
                  data: Data) -> "TrialPaths":
        """按 Experiment 的目录约定 + data 的投影参数计算全部路径。"""
        data_dir = exp.data_dir
        if data.type == "dataset":
            # dataset 类型：投影自 source，按 (source, input_format, parse, eval) 去重共享路径，
            # 同四元组的 trial 指向同一份 train/test，避免重复生成。
            src_basename = os.path.splitext(os.path.basename(data.source))[0]
            proj_key = (f"{src_basename}_{data.input_format}"
                        f"_{data.parse or 'none'}_{data.eval or 'none'}")
            return cls(
                train_data=os.path.join(data_dir, f"{proj_key}_train.jsonl"),
                test_data=os.path.join(data_dir, f"{proj_key}_test.jsonl"),
                bead_data="",
        ...
        if data.type == "bead":
            train_data = os.path.join(data_dir, f"trial{trial_id}_{trial_name}_bead.jsonl")
        else:
            ops_tag = (data.ops.replace('&', 'and').replace('|', 'or')
                       .replace('*', 'mul').replace('/', 'div'))
            mode_tag = f"{data.parse or 'none'}_{data.eval or 'none'}"
            train_data = os.path.join(
                data_dir,
                f"trial{trial_id}_{trial_name}_expr_{ops_tag}_{mode_tag}"
                f"_{data.input_format}_{data.repeat}_{data.start}-{data.end}.jsonl"
            )
        return cls(
            train_data=train_data,
            test_data=train_data if data.split <= 0 else train_data.replace('.jsonl', '_test.jsonl'),
            bead_data=os.path.join(data_dir, f"trial{trial_id}_{trial_name}_bead.jsonl"),
            best_model=os.path.join(exp.ckpt_dir, f"trial_{trial_id}.pth"),
            checkpoint=os.path.join(exp.ckpt_dir, f"trial_{trial_id}_ckpt.pt"),
            train_log=os.path.join(exp.log_dir, f"trial_{trial_id}_train.jsonl"),
            test_log=os.path.join(exp.log_dir, f"trial_{trial_id}_test.jsonl"),
...
class Experiment:
    ...
    词表按数据源去重后共享（见 model.vocab.ensure_experiment_vocab）。
    """
    name: str
    project_root: str
