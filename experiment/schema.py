...
4. 作用域严格两级，不可混淆：
      ...
      trial 级 name / heads / head_dims / theta_per_head / material / method
...
TRIAL_FIELDS = ("name", "heads", "head_dims", "theta_per_head", "material", "method")
TRIAL_DEFAULT_FIELDS = ("heads", "head_dims", "theta_per_head", "material", "method")
...
MATERIAL_TYPES = ("dataset", "expr", "bead")
...
# 各 material.type 的必填字段（其余字段按枚举校验）
REQUIRED_BY_TYPE = {
    "dataset": ("source",),
...
def _parse_material(errs: _Errors, raw: Any) -> Material | None:
    allowed = tuple(f.name for f in fields(Material))
    if not _check_keys(errs, raw, allowed):
        return None
    m = _coerce_fields(errs, Material, raw)
    type_ = m.get("type")
    if type_ not in MATERIAL_TYPES:
        errs.add(f"type={type_!r} 未知（可用: {', '.join(MATERIAL_TYPES)}）")
    else:
...
def parse_spec(raw: Any, where: str) -> ExperimentSpec:
    ...
    for i, t in enumerate(trials_raw):
        ...
        # material / method：trial_defaults 作基值，trial 写差量（扁平结构浅合并）。
        # 仅当 trial 与 defaults 都提供时合并；否则以提供的那一侧为准。
        material = None
        mraw = _merge_table(defaults.get("material"), t.get("material"))
        if mraw is None:
            sub.add("缺 material，且 trial_defaults 未提供")
        else:
            material = _parse_material(sub.scoped("material"), mraw)
        method = None
        ...
        if tname and material and method:
...
def _merge_table(defaults, trial):
    """material/method 合并：trial_defaults 作基值，trial 写差量。

    两侧都提供时浅合并（trial 同名键覆盖 defaults；material 为扁平结构，浅合并足够），
    两侧都缺失返回 None（交由调用方报「缺 material」）。每次返回新 dict，不污染 defaults，
