"""实验域模型 —— Experiment 聚合根及其组成部分（Material / Method / Trial / TrialPaths）。

2026-09-18 拆分：
  - 模型架构配置（BrainConfig / PosEmbConfig / ModelConfig）→ `model/config.py`
  - 训练记录（Record / RECORD_FIELDS）→ `model/record.py`（跨域共享：model_lm 生产、experiment 消费）
本模块只放实验域的静态配置与聚合根，不再被模型子系统依赖。

依赖方向（单向无环）：experiment.config → model.config / model.record。
